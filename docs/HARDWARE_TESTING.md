...
- [ ] Native Ubuntu environment ready
- [ ] ESP32 board with USB cable
- [ ] MCP2515 CAN module (optional for full tests)
...
cd /media/eddlai/DATA/ExoPulse_firmware
```
...
- VCC → 3.3V or 5V (check module specs), GND → GND
...
# Merge to master (if on feature branch)
git checkout master
git merge feature/can-bus-hardware-validation
```
...
---------------------------------
VCC     → 3.3V or 5V (⚠️ Check module specs - most need 5V!)
GND     → GND
SCK     → GPIO 18
MISO    → GPIO 19
MOSI    → GPIO 23
...
**⚠️ CRITICAL FINDING:** Most MCP2515 modules require **5V power**, not 3.3V! If loopback test fails with "Configuration Mode Failure", try 5V power.

#### Step 2.2: Prepare Loopback Test
...
#### Step 2.4: Monitor Loopback Test
...
4. Wrong power voltage (try 5V instead of 3.3V)
5. MCP2515 module defective

Solutions:
- Re-check all 6 wire connections
- Verify 3.3V or 5V on VCC pin with multimeter
- Try 5V power if 3.3V doesn't work
...
#### Step 3.1: Restore Production Code
...
#### Step 3.2: Build and Upload Production Firmware
...
3. **CRITICAL: Most MCP2515 modules need 5V, not 3.3V!**
4. Verify CS pin is GPIO 5
5. Try different MCP2515 module
6. Check crystal frequency (8MHz vs 16MHz)
```
...
### Hardware Validation Status Summary

**Date:** November 19, 2025
**Test Type:** Complete Hardware Validation
**Result:** ✅ **100% SUCCESS**
...
|---------------|--------|--------------|-------|
| LED Test | ✅ PASSED | 100% | ESP32 hardware verified |
| Loopback Test | ✅ PASSED | 100% | MCP2515 SPI communication verified |
| Serial Console | ✅ PASSED | 100% | All commands working |
| CAN Bus Communication | ✅ PASSED | 100% | 40/40 messages transmitted successfully |
| Motor Control | ⏭️ PENDING | N/A | Requires physical motor hardware |

#### Critical Configuration Findings

**MCP2515 Power Requirements:**
- **CRITICAL:** MCP2515 modules require **5V power**, not 3.3V!
- When powered with 3.3V: ❌ Initialization fails
- When powered with 5V: ✅ 100% success rate

**CAN Bus Physical Layer:**
- CAN Baud Rate: 500 KBPS
- Crystal: 8 MHz
- Termination: 120Ω resistors required on both ends
- Connections: CAN_H to CAN_H, CAN_L to CAN_L, common GND

#### Two-Board CAN Communication Test Results

**Test Configuration:**
- Board 1 (Sender): ESP32 NODEMCU-32S
- Board 2 (Receiver): ESP32 DevKit V1 DOIT
- Messages Sent: 40
...
- `docs/PROJECT_STATUS.md` - Build reports and deployment procedures
- `docs/MOTOR_MONITORING.md` - Motor monitoring tools
- `test/hardware_validation/CAN_BUS_TEST_RESULTS_FINAL.md` - Detailed CAN test results
- `test/hardware_validation/CAN_BUS_WIRING_CHECKLIST.md` - CAN wiring guide
