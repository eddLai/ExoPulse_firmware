#include "ExoBus.h"

#ifndef LED_BUILTIN
#define LED_BUILTIN 2
#endif

ExoBus::ExoBus() {
  // 建構函式留空，實際初始化在 begin()
}

ExoBus::~ExoBus() {
  if (can_) {
    delete can_;
    can_ = nullptr;
  }
}

bool ExoBus::begin() {
  pinMode(LED_BUILTIN, OUTPUT);
  digitalWrite(LED_BUILTIN, LOW);

  notify_("# ExoBus initializing...");

#ifdef EXOBUS_MCP2515_DEDALQQ
  notify_("# Using dedalqq/esp32-mcp2515 driver");
  // 初始化 SPI 裝置
  spi_bus_config_t busConfig = {};
  busConfig.miso_io_num = kSpiMiso_;
  busConfig.mosi_io_num = kSpiMosi_;
  busConfig.sclk_io_num = kSpiSck_;
  busConfig.quadwp_io_num = -1;
  busConfig.quadhd_io_num = -1;
  
  esp_err_t ret = spi_bus_initialize(HSPI_HOST, &busConfig, SPI_DMA_CH_AUTO);
  if (ret != ESP_OK) {
    char buf[64];
    snprintf(buf, sizeof(buf), "! SPI bus init failed: %d", ret);
    notify_(buf);
    return false;
  }
  notify_("# SPI bus initialized");

  spi_device_interface_config_t devConfig = {};
  devConfig.spics_io_num = kCanCsPin_;
  devConfig.clock_speed_hz = 1 * 1000 * 1000;
  devConfig.mode = 0;
  devConfig.queue_size = 1;
  
  ret = spi_bus_add_device(HSPI_HOST, &devConfig, &spiHandle_);
  if (ret != ESP_OK) {
    char buf[64];
    snprintf(buf, sizeof(buf), "! SPI device add failed: %d", ret);
    notify_(buf);
    return false;
  }
  notify_("# SPI device added");

  // 動態建立 MCP2515
  can_ = new CanDriver(&spiHandle_);
  notify_("# MCP2515 object created");
#else
  notify_("# Using coryjfowler/mcp_can driver");
  // 動態建立 MCP_CAN
  can_ = new CanDriver(kCanCsPin_);
#endif

  // CAN 初始化
  if (!canInit()) {
    notify_("! CAN init failed");
    return false;
  }
  notify_("# ExoBus ready");
  return true;
}

void ExoBus::poll() {
  if (!canReady_) return;
  uint32_t now = millis();
  if (now - lastReqMs_ >= kStatePeriodMs_) {
    requestState_();
    lastReqMs_ = now;
  }
  processRx_();
}

bool ExoBus::setTorque(int jointId, double torqueNm) {
  const double kNmMax = 30.0;
  if (!isfinite(torqueNm)) return false;
  if (torqueNm > kNmMax) torqueNm = kNmMax;
  if (torqueNm < -kNmMax) torqueNm = -kNmMax;
  int16_t iq = (int16_t)round(torqueNm * kNmToIq_);
  // 封包: 0xA1 + Iq (low/high) 與範例格式一致
  uint8_t d[8] = {0xA1,0,0,0,0,0,0,0};
  d[4] = iq & 0xFF;
  d[5] = (iq >> 8) & 0xFF;
  bool ok = canSend(motorBaseId_(jointId), d, 8);
  if (ok) logTx("TORQ", jointId, torqueNm);
  return ok;
}

// 新增：直接下 Iq（對齊 test0 的 0xA1，d[4..5] little-endian）
bool ExoBus::setTorqueIq(int jointId, int16_t iq) {
  const int16_t kMaxAbsIq = 300;
  int16_t orig = iq;
  if (iq >  kMaxAbsIq) iq =  kMaxAbsIq;
  if (iq < -kMaxAbsIq) iq = -kMaxAbsIq;
  
  if (orig != iq) {
    char buf[64];
    snprintf(buf, sizeof(buf), "! Iq clamped: %d -> %d", orig, iq);
    notify_(buf);
  }
  
  uint8_t d[8] = {0xA1,0,0,0,0,0,0,0};
  d[4] = (uint8_t)(iq & 0xFF);
  d[5] = (uint8_t)((iq >> 8) & 0xFF);
  bool ok = canSend(motorBaseId_(jointId), d, 8);
  if (ok) logTx("TORQ_IQ", jointId, (double)iq);
  return ok;
}

bool ExoBus::setSpeed(int jointId, float speedDps) {
  const float kMaxAbsSpeed = 50.0f;
  if (!isfinite(speedDps)) return false;
  float orig = speedDps;
  if (speedDps >  kMaxAbsSpeed) speedDps =  kMaxAbsSpeed;
  if (speedDps < -kMaxAbsSpeed) speedDps = -kMaxAbsSpeed;
  
  if (orig != speedDps) {
    char buf[64];
    snprintf(buf, sizeof(buf), "! Speed clamped: %.2f -> %.2f dps", orig, speedDps);
    notify_(buf);
  }
  
  int32_t sp = (int32_t)(speedDps * 100.0f);
  uint8_t d[8] = {0xA2,0,0,0,0,0,0,0};
  d[4] = (uint8_t)(sp & 0xFF);
  d[5] = (uint8_t)((sp >> 8) & 0xFF);
  d[6] = (uint8_t)((sp >> 16) & 0xFF);
  d[7] = (uint8_t)((sp >> 24) & 0xFF);
  bool ok = canSend(motorBaseId_(jointId), d, 8);
  if (ok) logTx("SPEED", jointId, speedDps);
  return ok;
}

// 新增：位置命令（對齊 test0 的 0xA3，值*100 放 d[4..7]）
bool ExoBus::setPosition(int jointId, float angleDeg) {
  if (!isfinite(angleDeg)) return false;
  int32_t p = (int32_t)(angleDeg * 100.0f);
  uint8_t d[8] = {0xA3,0,0,0,0,0,0,0};
  d[4] = (uint8_t)(p & 0xFF);
  d[5] = (uint8_t)((p >> 8) & 0xFF);
  d[6] = (uint8_t)((p >> 16) & 0xFF);
  d[7] = (uint8_t)((p >> 24) & 0xFF);
  bool ok = canSend(motorBaseId_(jointId), d, 8);
  if (ok) logTx("POS", jointId, (double)angleDeg);
  return ok;
}

bool ExoBus::stop() {
  // 以 TORQ 0 當暫時急停；可改獨立指令
  int jointId = 0; // 可廣播或迭代所有關節
  uint8_t d[8] = {0xA1,0,0,0,0,0,0,0}; // iq=0
  bool ok = canReady_ ? canSend(motorBaseId_(jointId), d, 8) : true;
  logTx("STOP", -1, 0);
  safeBlink(2, 60, 60);
  return ok;
}

bool ExoBus::callExo() {
  logTx("CALL", -1, 0);
  safeBlink(3);
  return true;
}

bool ExoBus::zeroAngle(int jointId) {
  uint8_t d[8] = {0x19,0,0,0,0,0,0,0};
  bool ok = canReady_ ? canSend(motorBaseId_(jointId), d, 8) : true;
  logTx("ZERO", jointId, 0);
  safeBlink(1);
  return ok;
}

bool ExoBus::zeroAll() {
  // 若有多關節可迴圈送 zeroAngle()
  bool ok = zeroAngle(0);
  logTx("ZERO_ALL", -1, 0);
  safeBlink(2);
  return ok;
}

void ExoBus::logTx(const char* what, int id, double v) {
  Serial.print(F("[TX] "));
  Serial.print(what);
  if (id >= 0) {
    Serial.print(F(" id="));
    Serial.print(id);
  }
  if (strcmp(what, "TORQ") == 0) {
    Serial.print(F(" Nm="));
    Serial.print(v, 3);
  }
  Serial.println();
}

void ExoBus::safeBlink(int times, int onMs, int offMs) {
  for (int i = 0; i < times; ++i) {
    digitalWrite(LED_BUILTIN, HIGH);
    delay(onMs);
    digitalWrite(LED_BUILTIN, LOW);
    delay(offMs);
  }
}
// ====== 新增實作區 ======
bool ExoBus::canInit() {
#ifdef EXOBUS_MCP2515_DEDALQQ
  notify_("# Resetting MCP2515...");
  can_->reset();
  
  notify_("# Setting bitrate 1Mbps@8MHz...");
  auto err = can_->setBitrate(CAN_1000KBPS, MCP_8MHZ);
  if (err != MCP2515::ERROR_OK) {
    notify_("# 1Mbps failed, trying 500kbps...");
    err = can_->setBitrate(CAN_500KBPS, MCP_8MHZ);
    if (err != MCP2515::ERROR_OK) {
      notify_("! Bitrate config failed");
      return false;
    }
    notify_("# Bitrate set to 500kbps");
  } else {
    notify_("# Bitrate set to 1Mbps");
  }
  
  can_->setNormalMode();
  notify_("# MCP2515 in NORMAL mode");
#else
  notify_("# Initializing MCP_CAN...");
  if (can_->begin(MCP_STDEXT, CAN_1000KBPS, MCP_8MHZ) != CAN_OK) {
    notify_("! MCP_CAN begin failed");
    return false;
  }
  can_->setMode(MCP_NORMAL);
  notify_("# MCP_CAN in NORMAL mode");
#endif
  canReady_ = true;
  return true;
}

bool ExoBus::canSend(uint32_t id, const uint8_t* data, size_t len) {
  if (!canReady_ || !can_) {
    notify_("! CAN not ready for send");
    return false;
  }
  if (len > 8) {
    notify_("! CAN frame too long");
    return false;
  }
#ifdef EXOBUS_MCP2515_DEDALQQ
  struct can_frame f;
  f.can_id  = id;
  f.can_dlc = (uint8_t)len;
  for (uint8_t i = 0; i < len; ++i) f.data[i] = data[i];
  auto err = can_->sendMessage(&f);
  if (err != MCP2515::ERROR_OK) {
    char buf[64];
    snprintf(buf, sizeof(buf), "! CAN send error: %d", (int)err);
    notify_(buf);
    return false;
  }
  return true;
#else
  byte rc = can_->sendMsgBuf(id, 0, (byte)len, (uint8_t*)data);
  if (rc != CAN_OK) {
    char buf[64];
    snprintf(buf, sizeof(buf), "! CAN send error: %d", (int)rc);
    notify_(buf);
    return false;
  }
  return true;
#endif
}

void ExoBus::requestState_() {
  uint8_t d[8] = {0x9C,0,0,0,0,0,0,0};
  // 假設只讀 jointId=0，可擴充迴圈
  canSend(motorBaseId_(0), d, 8);
}
void ExoBus::processRx_() {
  if (!canReady_ || !can_) return;
#ifdef EXOBUS_MCP2515_DEDALQQ
  struct can_frame f;
  while (can_->readMessage(&f) == MCP2515::ERROR_OK) {
    parseStateFrame_(f.can_id, f.can_dlc, f.data);
  }
#else
  while (can_->checkReceive() == CAN_MSGAVAIL) {
    unsigned long rxId;
    byte len;
    uint8_t buf[8];
    can_->readMsgBuf(&rxId, &len, buf);
    parseStateFrame_(rxId, len, buf);
  }
#endif
}
void ExoBus::parseStateFrame_(uint32_t rxId, uint8_t len, const uint8_t* buf) {
  if (len == 8 && buf[0] == 0x9C && rxId >= 0x140 && rxId < 0x160) {
    int jointId = (int)(rxId - 0x140);
    if (jointId < 0 || jointId >= 4) return;
    
    int8_t temperature = (int8_t)buf[1];
    int16_t iq_raw = (int16_t)(buf[2] | (buf[3] << 8));
    int16_t speed_raw = (int16_t)(buf[4] | (buf[5] << 8));
    uint16_t encoder = (uint16_t)(buf[6] | (buf[7] << 8));
    uint16_t encoder14 = encoder & 0x3FFF;
    
    float iq_A = (float)iq_raw * 33.0f / 2048.0f;
    float speed_dps = (float)speed_raw;
    float angle_deg = (float)encoder14 * 360.0f / 16384.0f;
    
    // 快取狀態
    lastState_[jointId].jointId = jointId;
    lastState_[jointId].angleDeg = angle_deg;
    lastState_[jointId].speedDps = speed_dps;
    lastState_[jointId].iqA = iq_A;
    lastState_[jointId].temperature = temperature;
    lastState_[jointId].valid = true;
    
    // 通知
    char buf2[128];
    snprintf(buf2, sizeof(buf2), 
             "[RX] J=%d ang=%.2f sp=%.1f iq=%.3f T=%d",
             jointId, angle_deg, speed_dps, iq_A, temperature);
    notify_(buf2);
  }
}

void ExoBus::notify_(const char* msg) {
  if (callback_) {
    callback_(msg);
  } else {
    Serial.println(msg);
  }
}

ExoBus::MotorState ExoBus::getLastState(int jointId) const {
  if (jointId >= 0 && jointId < 4) {
    return lastState_[jointId];
  }
  return MotorState{};
}