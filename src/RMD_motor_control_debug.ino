...
bool canSend(uint16_t id, const uint8_t d[8]) {
  ...
  Serial.printf("Sent CAN frame: ID=0x%X, Data={", id);
  for (int i = 0; i < 8; ++i) {
    Serial.printf(" 0x%02X", d[i]);
  }
  Serial.println(" }");
  return rc == CAN_OK;
}
...
bool waitReply(uint16_t expect_id, uint8_t expect_cmd, uint8_t out[8], uint32_t timeout_ms=200) {
  uint32_t t0 = millis();
  while (millis() - t0 < timeout_ms) {
    ...
    if (len == 8 && rid == expect_id && buf[0] == expect_cmd) {
      Serial.printf("Received CAN frame: ID=0x%lX, Data={", rid);
      for (int i = 0; i < 8; ++i) {
  ...
  Serial.println("Error: No reply within timeout.");
  return false;
}
...
bool enterClosedLoop(uint8_t id) {
  ...
  uint8_t r[8]; return waitReply(rx_id(id), 0x80, r, 100);  // Increased timeout
}
...
bool stopMotor(uint8_t id) {
  ...
  if (!canSend(tx_id(id), d)) return false;
  uint8_t r[8]; return waitReply(rx_id(id), 0x81, r, 100);  // Increased timeout
}
...
bool setTorque_A(uint8_t id, float iq_ampere) {
  ...
  if (!canSend(tx_id(id), d)) return false;
  uint8_t r[8]; return waitReply(rx_id(id), 0xA1, r, 100);  // Increased timeout
}
...
bool setSpeed_dps(uint8_t id, float dps) {
  ...
  pack_le32(d, sp_cnt);
  if (!canSend(tx_id(id), d)) return false;
  uint8_t r[8]; return waitReply(rx_id(id), 0xA2, r, 100);  // Increased timeout
}
...
bool setAbsPos_deg(uint8_t id, float deg) {
  ...
  pack_le32(d, ang_cnt);
  if (!canSend(tx_id(id), d)) return false;
  uint8_t r[8]; return waitReply(rx_id(id), 0xA4, r, 100);  // Increased timeout
}
...
bool addIncPos_deg(uint8_t id, float delta_deg) {
  ...
  pack_le32(d, inc_cnt);
  if (!canSend(tx_id(id), d)) return false;
  uint8_t r[8]; return waitReply(rx_id(id), 0xA8, r, 100);  // Increased timeout
}
...
bool readAngle_deg(uint8_t id, float &out_deg) {
  ...
  if (!canSend(tx_id(id), d)) return false;
  uint8_t r[8];
  if (!waitReply(rx_id(id), 0x92, r, 100)) return false;  // Increased timeout
...
bool readStatus(uint8_t id, int8_t &tempC, float &voltageV, uint16_t &errFlags, uint8_t &brakeState) {
  ...
  if (!canSend(tx_id(id), d)) return false;
  uint8_t r[8];
  if (!waitReply(rx_id(id), 0x9A, r, 100)) return false;  // Increased timeout
  tempC = (int8_t)r[1];
...
void demo(uint8_t id) {
  ...
  setTorque_A(id, 0.50f);  // Set torque to 0.50 A
  delay(500);
  stopMotor(id);
