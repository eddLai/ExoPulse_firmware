...
// Define this to skip LED/IMU/magnetometer initialization for minimal board
// Uses Fast64 star protocol for wireless communication to DK receiver
#define MINIMAL_FAST64 1
...
#ifdef MINIMAL_FAST64
// Simple polling-based ADC read for minimal board (no interrupts)
static volatile uint16_t minimal_adc_result = 0;
static uint8_t minimal_adc_initialized = 0;

// ADC sampling buffer - store 8 samples per packet (matching original format)
#define ADC_SAMPLES_PER_PACKET 8
static int16_t adc_sample_buffer[ADC_SAMPLES_PER_PACKET];
static uint8_t adc_sample_idx = 0;
static uint32_t last_sample_time = 0;
static uint8_t minimal_adc_data_id = 0;

void minimal_adc_init()
{
	// Simple single-shot ADC config for AIN4 (P0.28)
	// Matching original uMyo adc_read.c configuration for EMG signals
	NRF_SAADC->ENABLE = 1;
	NRF_SAADC->CH[0].PSELP = 5;  // AIN4 = P0.28 (adc_ain4)
	NRF_SAADC->CH[0].PSELN = 0;  // No negative input (single-ended)
	NRF_SAADC->CH[0].CONFIG =
		(0 << 0)  |  // RESP: no resistor (same as original)
		(1 << 4)  |  // RESN: pulldown resistor (same as original)
		(0 << 8)  |  // GAIN: 1/6 (adc_gain_1_6) - input range ±3.6V
		(0 << 12) |  // REFSEL: internal 0.6V (same as original)
		(1 << 16) |  // TACQ: 5us (adc_taq_5us, same as original)
		(0 << 20) |  // MODE: single ended
		(1 << 24);   // BURST: enabled (same as original)
	NRF_SAADC->RESOLUTION = 3;  // 14-bit (adc_res_14, same as original)
	NRF_SAADC->OVERSAMPLE = 7;  // 128x oversampling (adc_oversample_128x)
	NRF_SAADC->SAMPLERATE = 0;  // Task mode
	NRF_SAADC->RESULT.PTR = (uint32_t)&minimal_adc_result;
	NRF_SAADC->RESULT.MAXCNT = 1;
	minimal_adc_initialized = 1;
}

int16_t minimal_adc_read()
{
	if (!minimal_adc_initialized) return 0;
	...
	return (int16_t)minimal_adc_result;
}

// Sample ADC at 1kHz - call this from main loop
// Returns 1 when we have 8 samples ready
int sample_adc_1khz()
{
	uint32_t ms = millis();
	if(ms != last_sample_time) // 1ms has passed
	{
		last_sample_time = ms;
		if(adc_sample_idx < ADC_SAMPLES_PER_PACKET)
		{
			adc_sample_buffer[adc_sample_idx++] = minimal_adc_read();
		}
		if(adc_sample_idx >= ADC_SAMPLES_PER_PACKET)
		{
			return 1; // Buffer full, ready to send
		}
	}
	return 0;
}

// Prepare data packet in format matching original uMyo protocol
// This matches what the DK receiver expects
void prepare_minimal_data_packet()
{
	uint8_t idx = 0;
	...
	data_packet[idx++] = packet_id;
	data_packet[idx++] = 0; // Length - fill in at end
	...
	uint8_t send_cnt = 8;
	data_packet[idx++] = 80 + send_cnt;  // Data type indicator

	// param_id = battery
	data_packet[idx++] = 0;  // param_batt
	data_packet[idx++] = 200; // battery_level (dummy value ~4V)
	data_packet[idx++] = 101; // version_id
	data_packet[idx++] = 0;

	// ADC data ID
	data_packet[idx++] = minimal_adc_data_id++;

	// 8 ADC samples (16-bit each)
	for(int n = 0; n < ADC_SAMPLES_PER_PACKET; n++)
	{
		data_packet[idx++] = (adc_sample_buffer[n] >> 8) & 0xFF;
		data_packet[idx++] = adc_sample_buffer[n] & 0xFF;
	}

	// Spectrum data (4 values) - dummy zeros
	for(int x = 0; x < 4; x++)
	{
		data_packet[idx++] = 0;
		data_packet[idx++] = 0;
	...
	// Quaternion (4 values) - identity quaternion (1,0,0,0) scaled
	data_packet[idx++] = 0x7F; data_packet[idx++] = 0xFF; // qw = 32767
	data_packet[idx++] = 0; data_packet[idx++] = 0; // qx = 0
	data_packet[idx++] = 0; data_packet[idx++] = 0; // qy = 0
	data_packet[idx++] = 0; data_packet[idx++] = 0; // qz = 0

	// Accelerometer (3 values) - zeros
	data_packet[idx++] = 0; data_packet[idx++] = 0; // ax
	data_packet[idx++] = 0; data_packet[idx++] = 0; // ay
	data_packet[idx++] = 0; data_packet[idx++] = 0; // az

	// Yaw/Pitch/Roll (3 values) - zeros
	data_packet[idx++] = 0; data_packet[idx++] = 0; // yaw
	data_packet[idx++] = 0; data_packet[idx++] = 0; // pitch
	data_packet[idx++] = 0; data_packet[idx++] = 0; // roll

	// Magnetometer (3 values) - zeros
	data_packet[idx++] = 0; data_packet[idx++] = 0; // mx
	data_packet[idx++] = 0; data_packet[idx++] = 0; // my
	data_packet[idx++] = 0; data_packet[idx++] = 0; // mz

	data_packet[1] = idx; // Set length

	// Reset sample buffer for next batch
	adc_sample_idx = 0;
}
#else
...
#ifdef MINIMAL_FAST64
	// For minimal board without external crystal, use timeout and fallback to internal RC
	volatile uint32_t timeout = 100000;
	while (NRF_CLOCK->EVENTS_HFCLKSTARTED == 0 && timeout > 0) { timeout--; }
	// If no external crystal, the internal HFXO will be used
	// Fast64 star protocol can work with internal RC oscillator
#else
...
uint8_t radio_mode = radio_mode_fast64; // Use Fast64 star_protocol for dongle communication
...
int main(void)
{
	...
	// NRF_UICR->NFCPINS = 0; // REMOVED: Direct UICR writes cause HardFault on nRF52810
...
#ifdef MINIMAL_FAST64
	// Skip LED, IMU, magnetometer initialization for minimal board
	// Use Fast64 star protocol directly
	radio_mode = radio_mode_fast64;

	// Initialize simple polling-based ADC for reading analog input
	minimal_adc_init();

	uint32_t unit_id = NRF_FICR->DEVICEID[1];

	// Initialize Fast64 star protocol (channel 21, 1000kbps, 2000us phase, peripheral mode)
	star_init(21, 1000, 2000, 0);
	star_set_id(unit_id);
	...
	int cur_rf32_dt = 10;
	uint8_t zerow_test_passed = 1; // Skip gyro calibration
#else
...
//	star_init(21, 1000, 2000, 0);
	...
	while(1)
	{
		NRF_WDT->RR[0] = 0x6E524635; //reload watchdog
#ifdef MINIMAL_FAST64
		// Minimal Fast64 mode with 1kHz ADC sampling
		// Sample ADC every 1ms, send packet when buffer is full (8 samples)
		star_loop_step();  // Process star protocol

		if(sample_adc_1khz())  // Returns 1 when 8 samples ready
		{
			prepare_minimal_data_packet();
