...
/* Forward declare ISR handlers for Zephyr */
ISR_DIRECT_DECLARE(timer2_isr_handler)
{
...
ISR_DIRECT_DECLARE(timer3_isr_handler)
{
	...
	if(NRF_TIMER3->EVENTS_COMPARE[1])
	{
		...
		if(timed_subevent1_enabled && timed_subevent1)
			timed_subevent1();
	}
	if(NRF_TIMER3->EVENTS_COMPARE[2])
	{
		NRF_TIMER3->EVENTS_COMPARE[2] = 0;
		if(timed_subevent2_enabled && timed_subevent2)
			timed_subevent2();
	}
	ISR_DIRECT_PM();
	return 1;
...
static int timer2_irq_connected = 0;
static int timer3_irq_connected = 0;
...
void time_start()
{
	...
	if (!timer2_irq_connected) {
		IRQ_DIRECT_CONNECT(TIMER2_IRQn, 2, timer2_isr_handler, 0);
		timer2_irq_connected = 1;
	}
...
void schedule_event(uint32_t steps_dt, void (*tm_event)(void), int repeated)
{
	...
	/* Zephyr IRQ connect - only once */
	if (!timer3_irq_connected) {
		IRQ_DIRECT_CONNECT(TIMER3_IRQn, 3, timer3_isr_handler, 0);
		timer3_irq_connected = 1;
	}
...
void schedule_event_adjust(int dt)
{
	...
	timed_event_adjustment_state = 1;
}
