...
/* Define the ISR handler first so it can be used in IRQ_DIRECT_CONNECT */
ISR_DIRECT_DECLARE(radio_isr_handler)
{
	...
	if(NRF_RADIO->EVENTS_END)
	{
		...
		else if(current_mode == rm_tx2rx)
		{
			...
			while(NRF_RADIO->STATE != st_radio_rxidle && NRF_RADIO->STATE != st_radio_rx) ;
...
static int radio_irq_connected = 0;

void rf_disable()
{
	if(NRF_RADIO->STATE == st_radio_disabled) return;
...
void rf_init_ext(int channel, int speed, int crc_len, int white_en, int s1_sz, int added_length, int max_length)
{
	...
	/* Note: nRF52840 doesn't support 250Kbit mode */
	if(speed == 1000)
    ...
    NRF_RADIO->PREFIX0     = 0x44332211UL;
    NRF_RADIO->PREFIX1     = 0x88776655UL;
    NRF_RADIO->BASE0       = 0x0EE60DA7UL;
    NRF_RADIO->BASE1       = 0x0EE60DA7UL;
    NRF_RADIO->TXADDRESS   = 0x0UL;
    NRF_RADIO->RXADDRESSES = 0b00000001;
	...
	NRF_RADIO->CRCINIT = 0b10101010;
	NRF_RADIO->CRCPOLY = 0b100000111;
	NRF_RADIO->EVENTS_DISABLED = 0;
	NRF_RADIO->EVENTS_END = 0;
	...
	/* Zephyr IRQ connect - only once */
	if (!radio_irq_connected) {
		IRQ_DIRECT_CONNECT(RADIO_IRQn, 0, radio_isr_handler, 0);
		radio_irq_connected = 1;
	}
